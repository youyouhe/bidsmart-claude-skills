...
requires: [python:docx, python:fitz, python:pdfplumber(可选), docscan]
---
...
| **暗标规范** | （检出暗标时概括规则数量与核心要求；未检出写"本项目未采用暗标评审"） | （检出时摘录关键条款原文；未检出写检索依据） | （来源） | 检出时详见本报告"暗标格式规范"节；违反=否决投标 |
...
#### 暗标检测与格式规范提取（条件触发，检测动作必须执行）

招标文件中出现"暗标""匿名评审""盲评""不得出现投标人名称/身份信息""技术标暗标"等表述时，本项目采用暗标评审——暗标格式违规通常直接否决投标，必须将格式规则逐条提取为独立小节。

**检测方式**：对已提取的全文检索关键词（`暗标|匿名|盲评|身份信息|不得出现.*(名称|单位|标识)`），命中任一即按检出处理。

**检出时**，在合规注意事项表之后增加独立小节：

```markdown
### 暗标格式规范

| 序号 | 检查项 | 格式要求（原文） | 适用范围（原文册别/部分名称） | 违规后果 | 精确来源 |
|------|--------|----------------|------------------------------|----------|---------|
| 1 | （如"字体字号"） | （逐字引用，如"宋体四号"） | （如"二、商务技术文件（暗标部分）"） | （原文；默认"否决投标"） | Table N Row M |
```

**提取要点：**
- 逐条抄录：纸张、字体字号、颜色、行距、对齐、页眉页脚、目录、图表样式、文件命名规则等，每条单独成行
- "不得出现投标人名称/标识/人员姓名"等身份隐匿要求单独成行，并注明覆盖范围（正文/图表/页眉页脚/文件名/元数据）
- 违规后果按原文逐字引用；原文未写明时填"否决投标（推定）"并在风险清单中列出
- 适用范围必须写明该规则作用于哪一册/哪部分，使用招标文件原文的册别名称（暗标通常只约束技术标所在册）
- **统计口径保护**：适用范围列**禁止单独填写"商务标"/"技术标"字样**——`| 商务标 |` / `| 技术标 |` 单元格会被统计脚本计入 business_files/tech_files，污染完成摘要计数；须写原文册别名称（如"二、商务技术文件"）

**未检出时**：合规表"暗标规范"行填"本项目未采用暗标评审"并注明检索依据，不输出本小节。
...
- **暗标检测是否已执行**（全文检索"暗标/匿名/盲评/身份信息"关键词；检出则"暗标格式规范"节已逐条提取，未检出则合规表已注明"本项目未采用暗标评审"及检索依据）
- **风险清单是否已汇总**（四类风险分级列出、每条有精确来源，冲突处理中未裁决的质询项全部入表）
...
将分析过程中已识别的风险聚合为一张风险登记册，写入分析报告"风险清单"节。风险素材已存在于报告各处（★实质性条款、暗标格式规则、未裁决的冲突质询项、数据验证发现的矛盾、合规注意事项备注），本节的职责是**汇总分级**，不是新增判断——每条风险必须能回溯到报告中已有的提取结果，禁止凭经验补写招标文件里没有的风险。

**风险类别固定为四类：**

| 类别 | 定义 | 典型来源 |
|------|------|---------|
| 废标风险 | 违反即否决投标的硬性条款 | ★实质性条款、暗标格式规则、密封/签署/份数/递交时限等合规项 |
| 评分风险 | 不废标但直接丢分的短板 | 客观分缺材料（证书/业绩/社保）、需提前办理的硬性材料、主观分竞争激烈项 |
| 合规风险 | 不必然废标但可能被质疑/投诉 | 资格边缘条件、联合体/分包限制、业绩关联关系限制 |
| 流程陷阱 | 程序性失误风险 | 保证金到账截止≠递交截止、平台上传时限、澄清/质疑截止时间、多密封包要求 |
...
| 类别 | 风险描述 | 等级 | 应对建议 | 精确来源 |
|------|---------|------|---------|---------|
| 废标风险 | （具体条款+触发条件） | 高 | （具体动作） | Table N Row M |
```

**填写要求：**
- 等级：高/中/低。废标风险默认"高"；评分风险按分值权重定级（高分值缺材料=高）
- 每条必须标注精确来源（`Table{N} Row{M}` / `段落{N}`），与其他章节同一标准；无法在原文定位来源的条目禁止写入
- 应对建议必须具体可执行（如"提前办理XX认证，常规周期约4-6周"），禁止"注意""重视"等空话
- 冲突处理中**未能自裁决的质询项全部进入本表**，按性质归类，等级不低于"中"
...
1. **最权威**：对原始 docx 直接跑权威脚本（python-docx 读表格结构，输出每个表的行/列/★/▲ 计数与表头预览）：
   ```bash
   python3 $SKILLS_BASE_PATH/bid-manager/scripts/report_stats.py tables <招标文件.docx>
   ```
2. **次优**：人工逐行读表格数（打开原表格，一行一行数，不跳读、不估算）
3. **仅用于交叉校验，不可单独作为最终依据**：`grep -c` 关键字/符号出现次数——如与方法1/2结果不一致，**以方法1/2为准**并回查 grep 模式为何漏行/多计

生成阶段的计数和自检阶段的复核**必须使用同一权威方法**重新各算一次，不可让自检直接照抄生成阶段的数字"过一眼觉得像就通过"。

### 统计 JSON 块（强制，报告末尾）

分析报告**末尾必须嵌入统计 JSON 块**，数字由脚本从正文计数生成，**禁止手写**：

```bash
# 写完正文后：生成统计块并追加到报告末尾
python3 $SKILLS_BASE_PATH/bid-manager/scripts/report_stats.py fill 分析报告.md --kind analysis >> 分析报告.md
# 自检：统计块与正文不一致则退出码 1，必须修正
python3 $SKILLS_BASE_PATH/bid-manager/scripts/report_stats.py check-report 分析报告.md
```

- `business_files`/`tech_files` = 正文中 `| 商务标 |` / `| 技术标 |` 的表格行计数（脚本权威）
- 历史事故：分析师曾手写"商务 20/技术 7"（实际 19/9）——此后统计数字只认脚本产物
- 核实阶段（bid-verification）会用同一脚本复核该块，不一致按错误处理
...
暗标评审: {是|否}（是则注明暗标规则条数）
风险清单: {N}条（废标风险X/评分风险X/合规风险X/流程陷阱X）
输出文件: 分析报告.md
状态: SUCCESS
--- END ---
```
