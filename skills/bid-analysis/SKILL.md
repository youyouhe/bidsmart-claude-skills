...
Write 工具是**覆盖模式**，多次调用会丢失前面的内容。生成分析报告时必须使用以下方式：

### 使用 bash cat 分段 append（推荐）
...
从采购文件的"投标文件格式"章节提取文件册别组织方式。大多数政府采购项目要求将投标文件分为多册（如"资格证明文件"和"商务技术文件"两册），每册有独立封面。

```markdown
### 册别结构
| 册别 | 名称 | 封面格式 | 包含内容 |
|------|------|----------|----------|
| 第一册 | 投标文件（资格证明文件） | 实质性格式/非实质性格式 | 营业执照、资格声明书、中小企业声明函等 |
| 第二册 | 投标文件（商务技术文件） | 实质性格式/非实质性格式 | 投标书、报价表、偏离表、技术方案等 |
（如采购文件未明确分册，标注"单册"）
```

**关键要求：**
- 每册包含的附件清单必须明确列出，下游 skill 据此决定文件归属
- 标注哪些文件属于哪一册（如"中小企业声明函"应放在资格证明文件册中）
- 如采购文件使用电子投标客户端，标注哪些是客户端内置模板、哪些需上传附件
