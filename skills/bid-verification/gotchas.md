...
### grep -c 被当作表格行数的权威依据，核实阶段复用了生成阶段的错误口径

**问题**：正则/grep 校验表格行数会漏行或多计——表格有合并单元格（rowspan）或单元格内换行时，Markdown 渲染后一行表格内容会跨多个物理行，grep 数出的"行数"不等于真实表格行数。更严重的是，核实阶段本应是独立的第二道关卡，但实际操作中直接拿分析报告里已经算好的数字去核对，而不是用权威方法独立重新数一遍——等于自检和被检用了同一个可能错误的口径，起不到交叉验证作用。

**真实案例（飞机区域仿真系统采购项目，2026-JH18-F1002）**：技术评审表用星号/关键字 grep 数出37项，人工逐行读表格实际是35项。S2 核实阶段第一次核对时误用了错误的"37"，直到人工逐行数第二遍才发现问题并修正。

**解决**：
1. 核实评审表/清单表等行数类数据时，权威方法是对原始 docx 直接读 `len(doc.tables[N].rows)`（python-docx）或人工逐行数；`grep -c` 只能作为"数字对不上、需要回查"的信号，不能作为核实结论
2. 核实阶段的计数**必须独立执行**（重新读原表格/重新跑脚本），不能直接采信分析报告已给出的数字后仅做形式比对——如果核实阶段图省事复用了分析报告的计数结果，一旦分析报告本身算错，核实完全无法发现
3. grep 结果与逐行读表结果不一致时，以逐行读表为准，并在核实报告中记录差异原因

**验证方法**：核实报告中涉及表格行数的核实项，检查计数方法是否写明"独立读表/独立跑脚本"，仅写"与分析报告一致"而未说明独立计数过程的，视为核实未完成。

*（来自飞机区域仿真系统应用软件开发服务采购项目（2026-JH18-F1002）一键投标复盘，注入时间：2026-08-01）*
