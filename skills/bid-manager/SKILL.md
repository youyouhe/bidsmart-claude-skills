...
**此阶段仍必须暂停等待用户输入，但优先顺序变了：先尝试从 MaterialHub 查候选数据供用户确认，查不到或属于决策类信息才让用户从头填写。** 具体执行 bid-commercial-proposal 的"步骤2：收集公司信息"流程（该 skill 已内建 MaterialHub 优先查询逻辑），而不是在本 skill 里另起一套收集逻辑：
