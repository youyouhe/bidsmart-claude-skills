...
分析报告的"响应文件组成/格式文件清单"本身可能漏列（历史事故：自报 15 个，实际招标有 28 个），故必须以**招标原始格式模板**为闭环基准，不得只信分析报告：

1. 枚举招标 `采购文件附件/`（或等价格式模板目录）下的全部模板（`.docx/.pdf/.xlsx`，读首行/标题取模板名），作为"招标要求文件"全集 R。
2. 枚举 `响应文件/*.md`（取标题/文件名），作为"实际生成"全集 A。
3. 逐项为 R 中每个模板在 A 中找对应（按模板名关键词匹配；特别注意"技术力量1~5"是 5 个独立模板、"企业规模1"、"投标（报价）信息汇总表"等易漏项）。
4. 任一招标模板在 A 中**找不到对应输出** → 记 🔴，问题描述写明"招标要求《XX》无对应响应文件"，并在 target_skill 注明推断的漏生成方（bid-tech-proposal / bid-commercial-proposal / bid-analysis 漏列）。
5. 多个招标独立模板的内容被合并进同一输出文件（如"技术力量1-5"五模板→一个文件）→ 记 🔴（若该组评分项≥3分）或 🟡，提示"按招标结构拆分为 N 个独立文件"。

此项即使分析报告自称"格式清单完整"也必须独立执行（闭环不依赖分析报告）。
