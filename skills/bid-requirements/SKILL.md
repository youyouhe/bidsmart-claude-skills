...
读取以下文件：
1. `项目文档/01-需求分析/_metadata.md` — 项目元数据（含拆分计划）
2. 技术响应表中目标系统的功能清单
3. 技术方案中该系统的架构描述
...
对每个子模块，读取该子模块的功能点详情，按 1.3 模板逐功能展开，
输出 `{序号}-{子模块名称}.md`。
...
**单文件行数控制：建议不超过 500 行。如某子模块功能点过多（>8个），考虑进一步拆分。**
