...
- ▲标注条目（截图+盖章要求）
- 功能条目总数（用于后续核对）
- **非功能需求类别清单**：分析报告"技术需求"节中除功能需求外的类别（性能要求、安全需求、数据资源、接口要求、建设原则等），逐类列出并标注是否含▲条目——供 4.1"需求覆盖补章"校验使用
...
- **页码类列（如"报价文件位置页码"，仅当分析报告"格式文件清单"中该表列结构含此类列时必须填写）**：编写阶段 Word 尚未生成、页码不可知，本列填**锚点章节标题**作为占位——S8（bid-assembly）据此建映射、S10（bid-md2doc）用 DocScan crossref 自动回填真实页码：
  - 该条的详细展开写在响应表之外（如服务方案/技术方案某章）→ 填该章 **H2 标题原文**（精确照抄、含编号，如 `3.2 接口设计`）；引用多个章节用 `、` 分隔
  - 响应内容在本行内已自包含（响应列已完整作答，正文无更详细展开）→ 填 `本表`。**但▲实质性条款、性能/安全/建设原则等非功能需求类别的条目不得填"本表"**——评委期望在正文看到展开论述；对应章节不存在时，先按 4.1"需求覆盖补章"在技术方案中补写该章节，再填其标题
  - 引用其他册的章节 → 填 `见{册名原文}`（crossref 无法跨 docx，保持文字指引）
  - **禁止**模糊指代："详见方案""见服务方案""本附件+服务方案"等非标题原文表述——S8 无法解析为锚点将跳过该行，最终 Word 中该格没有页码
...
5. **需求覆盖补章（评分维度之外的必备章节，强制校验）**：评分维度逆向设计覆盖的是"得分点"，但招标技术需求中常有不占分的类别——性能要求、安全需求、数据资源、接口要求、建设原则等。它们不对应任何评分子维度，却是响应表行（尤其▲实质性条款）的锚点去向；缺章会导致响应表该行的页码列只能填"本表"，评委翻开正文找不到任何展开。**补章规则**：
   - 取出步骤 1 提取的"非功能需求类别清单"，逐类确认技术方案（或其他技术归属文件）中是否已有章节承接
   - **凡含▲实质性条款的类别、凡响应表行需要引用的类别 → 必须有章节**；缺失则在技术方案中补写对应章节（讲实现机制/指标保障措施，按 4.2 详细程度要求编写，禁止两行敷衍）
   - 已由功能章节自然覆盖的类别不重复补章；招标技术需求中本就不存在的类别不补（以分析报告为准，不凭经验虚增）
...
- [ ] 每个写作型评分因素都有对应输出文件
- [ ] 每个评分子维度都有独立章节（无遗漏维度）
- [ ] **🆕 需求覆盖补章检查**：非功能需求类别清单中凡含▲条目、凡被响应表页码列引用的类别，技术方案中均有对应章节；响应表页码列无"本表"滥用于▲/非功能需求条目
- [ ] 所有▲标注功能有截图占位 + 盖章标注
- [ ] 全生命周期成本合同期费用 = 报价金额
- [ ] 响应说明无空泛描述（逐条抽检）
- [ ] 进度计划在交付期限内
