...
对于**软件项目**（采购内容包含系统/平台/应用开发），在编写功能方案章节时，**必须为每个系统/功能模块预留 POC 截图占位符**。下游 `bid-poc` 会按需求规格生成 POC 原型，`bid-poc-screenshots` 会自动截取并替换这些占位符。

**占位符格式（严格遵守，下游按此精确匹配）：**

```
【此处插入{系统标识}功能截图】（截图需加盖公章）
```

其中 `{系统标识}` = **系统编号 + 系统名称**。系统编号与名称**一律取自 `system_decomposition.json`**（全流水线单一事实源，由 bid-system-decomp 生成），禁止自行编号/命名——这样才能与需求规格书 / POC 子目录命名一致：
- ✅ `【此处插入S04性能预报系统功能截图】`
- ✅ `【此处插入S01数据采集系统功能截图】`
- ❌ `【此处插入功能截图】`（缺少系统标识，无法匹配）
- ❌ `【此处插入系统截图】`（太模糊）

**放置位置规则——对应到具体功能章节（小节），不是随意塞一处：**

每个系统在技术方案中都有独立的功能描述小节（如 `### 2.4 性能预报系统` / `## 4.2 S04 性能预报功能域`）。占位符放在**该系统功能描述小节的末尾**——即写完技术方案正文、图表之后，进入下一个系统之前：
...
**如何确定哪些系统需要预留占位符（按优先级，禁止自行划分系统）：**

1. **首选 `system_decomposition.json`**（工作目录根，由 bid-system-decomp 生成）：遍历其 `systems[]`，对每个 `ui_prototype: true` 的系统，**原样复制其 `screenshot_placeholder` 字段**到对应功能小节末尾。系统编号、名称、占位符文本三者均以 json 为准，一个字不改。
2. **json 不存在时**（独立调用 / 非软件标的的兜底）：从 `分析报告.md` 的采购需求中提取系统清单，仅对有 UI 界面的软件系统预留；并在完成状态块中标注 `⚠️ system_decomposition.json 缺失，系统编号未经契约校验`，便于 bid-manager / bid-assembly 发现系统分解阶段被跳过。
...
### 2.1 数据采集系统
（方案正文...）
【此处插入S01数据采集系统功能截图】（截图需加盖公章）

### 2.2 建模计算引擎
（方案正文...）
【此处插入S02建模计算引擎功能截图】（截图需加盖公章）

### 2.3 三维可视化系统
（方案正文...）
【此处插入S03三维可视化系统功能截图】（截图需加盖公章）
```

**自检**：写完技术方案后，运行 `grep -c '【此处插入.*功能截图】' 响应文件/*.md`，确认占位符数量 = 需要截图的系统数量。漏预留会导致下游截图无处安放。

**🆕 图表占位符格式自检（与功能截图占位符自检并列，强制）：** 图表占位符必须严格使用 `此处插入XX图` 格式（与上方"图表生成规范"模板一致），下游 `bid-mermaid-diagrams` 按此精确匹配后渲染。
- **禁止把"插入"改成同义动词**：禁止出现 `展示XX图` / `暂留XX图` / `预留XX图` / `添加XX图` / `生成XX图` 等变体——下游匹配不到 → 该图永远不会被渲染
- **禁止外层加 markdown 加粗**：禁止写成 `**【此处插入XX图】**` 或 `**此处插入XX图**`

**自检（编写后必须执行，命中数应为 0）：**
```bash
# 检索"插入"被替换为同义动词的变体，以及被 markdown 加粗包裹的占位符
grep -nP '【此处(展示|暂留|预留|添加|生成).*(图|截图)|此处(展示|暂留|预留|添加|生成).*(图|截图)|\*\*【?此处插入' 响应文件/*.md
```
- 命中行必须改回 `【此处插入XX图】` 标准格式（不加粗、用"插入"二字）
- 自检未归零不得结束本文件编写
...
软件项目的技术方案**必须**在功能章节预留 POC 截图占位符，格式与放置规则见 [4.2.1 功能截图占位符预留](#421-功能截图占位符预留软件项目必备)。

占位符格式：`【此处插入{系统标识}功能截图】（截图需加盖公章）`，{系统标识}含系统编号+名称（如 S04性能预报系统），放在该系统功能小节正文末尾。

实际的截图截取与替换由下游 `bid-poc-screenshots` 阶段自动完成（调用 `poc_screenshot` 工具），本阶段只负责**预留位置**。
...
技术响应表条目数: {N}
▲截图占位数: {N}（已预留占位符，由下游 bid-poc + bid-poc-screenshots 自动完成截图和替换）
图表占位数: {N}（已预留占位符+Mermaid代码，由下游 bid-mermaid-diagrams 自动渲染）
评分子维度覆盖: {已覆盖}/{总数}
输出目录: 响应文件/
状态: SUCCESS
--- END ---
...
**🔧 流水线自动处理（本阶段无需关注）：**
- ✅ POC 功能截图 → 下游 `bid-poc` + `bid-poc-screenshots` 自动生成并替换
...
**禁止**将 POC 截图或图表列在"需要人工补充"清单中——这些是流水线自动完成的。如果混在一起，用户会困惑为什么"还在提示需要截图"。
