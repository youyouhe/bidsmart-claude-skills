...
- 子系统/模块清单及其功能条目
- ▲/★标注条目及其含义（从分析报告中读取，不同项目含义不同）
- 证明材料要求（是否要求截图、盖章、页码引用等 — 以采购文件原文为准）
- 功能条目总数（用于后续核对）
...
- **▲/★标注功能**：根据采购文件的实际要求处理。**不同项目对 ▲/★ 的要求不同**：
  - 有些项目要求截图+盖章（此时标注 `【此处插入XX功能截图】（截图需加盖公章）`）
  - 有些项目仅要求在偏离表中标明页码和位置（此时无需截图占位）
  - **必须从分析报告中读取具体要求，禁止硬编码假设**
- **禁止**：空泛描述（"支持该功能""满足要求"）、复制粘贴需求原文作为响应

#### 3.3 编写后核对

- 统计响应表条目数，与原始需求表条目数比对
- 如采购文件要求截图，确认每个▲条目都有截图占位标注；如仅要求页码引用，确认偏离表格式正确
- 确认无空响应行
...
- [ ] 每个写作型评分因素都有对应输出文件
- [ ] 每个评分子维度都有独立章节（无遗漏维度）
- [ ] ▲/★标注功能的证明材料处理方式与采购文件要求一致（截图/页码引用/其他）
- [ ] 全生命周期成本合同期费用 = 报价金额
- [ ] 响应说明无空泛描述（逐条抽检）
- [ ] 进度计划在交付期限内
- [ ] 质保期/维护期与商务条款一致
...
| ▲/★证明材料处理不当 | 根据采购文件实际要求调整（截图/页码引用/其他） |
...
### ▲/★标注功能的证明材料
**数据驱动，不硬编码。** 不同项目对 ▲/★ 标注的要求差异很大：
- 部分项目要求截图+盖章 → 添加 `【此处插入XX功能截图】（截图需加盖公章）`
- 部分项目仅要求在偏离表中标明页码 → 无需截图，只需确保偏离表有正确的页码引用
- 部分项目 ★ 为实质性指标（不满足=废标），▲ 为重要指标（扣分），无标记为一般指标
- **必须从分析报告中读取采购文件对 ▲/★ 的具体定义和证明材料要求**
...
| ▲/★证明材料处理错误 | 扣分或废标 | 从分析报告读取具体要求，不硬编码截图假设 |
...
- 表格使用 Markdown 表格语法
- 图片占位：`【此处插入XXX图】`
- 截图占位（仅当采购文件明确要求时）：`【此处插入XX功能截图】（截图需加盖公章）`
- 签章标记：`（盖章）` `（签字）`
- 每个文件开头：`# 附件N：标题`
