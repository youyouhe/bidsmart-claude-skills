...
const TYPE_LABELS = {
  frontend: '前端', backend: '后端', database: '数据库', cloud: '云服务',
  security: '安全组件', messagebus: '消息总线', external: '外部系统',
};
function renderLegend() {
  ...
  const y = legendY();
  let x = layout.margin;
  const parts = [`        <text x="${x}" y="${y - 13}" class="t-primary" font-size="9" font-weight="600">图例</text>`];
